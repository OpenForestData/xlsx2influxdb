...
##
def convertAndPrepareToCsv(inputfilename):
...
def loadCsv(inputfilename, servername, user, password, dbname, metric, 
    ...
    convertAndPrepareToCsv(inputfilename)
    ...
    # open csv
    datapoints = []
    inputfile = open("file_"+datetime.datetime.now().strftime("%Y_%m_%d_%H_%M")+".csv")
    count = 0
    ...
    print('Done')
    os.remove("file_"+datetime.datetime.now().strftime("%Y_%m_%d_%H_%M")+".csv")
